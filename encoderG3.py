...
BASE_OUTPUT_DIR = "causal_manipulation_L14_ultimate3"
...
def forward_pass_with_swap_and_decode(
    ...
    for t in range(seq_len):
        ...
        if decoding_active:
            ...
            #######################SHAAAAMMMM########################################
            """
        ####here is where the forward pass was########
    ############NOT SHAM####################
    return (
    ...
    for i in range(len(timesteps)):
        if i % (LOOP_LENGTH_TO_ANALYZE // 2) == 0:
             plt.text(timesteps[i], accuracy_over_time[i] + 0.02, 
                     decoder_label, ha='center', size='x-small')

    plt.axvline(x=t_intervene, color='r', linestyle='--', label=f'Swap Intervention at t={t_intervene}')
    plt.title(f'obs_y Decoding Accuracy After Slot {slot_k} Swap Manipulation', fontsize=16)
    plt.xlabel('Timestep (t)', fontsize=14)
    plt.ylabel(f'Accuracy of D{slot_k:02d} Decoding Added obs_y', fontsize=14)
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.ylim(-0.05, 1.05)
...
def run_swap_manipulation_for_slot(
    ...
    correct_decodes = (decoder_preds == obs_y_targets.unsqueeze(1)) ############not sham##############
...
def main():
    ...
    try:
        ...
        import traceback
        print(f"--- ERROR DURING SWAP EXPERIMENT ---")
        traceback.print_exc()
        print(f"------------------------------------")
        print(f"Error was: {e}")
        print("Skipping SWAP experiments.")
